...
export function DiagnosticReportForm({
  ...
  // Calculate remaining unused diagnostic report codes
  ...
  const { mutate: createDiagnosticReport, isPending: isCreatingReport } =
    useMutation({
      ...
      onSuccess: () => {
        ...
        // Reset selected code to allow selecting next code
  ...
  useEffect(() => {
    ...
    if (latestReport) {
      // If we have a new report, expand the section to show it
      setIsExpanded(true);
    }
  }, [diagnosticReports]);
  ...
  function handleCreateReport() {
    // Allow creating multiple reports (one per code)
